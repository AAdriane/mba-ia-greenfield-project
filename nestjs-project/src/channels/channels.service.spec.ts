...
/** The subset of `EntityManager` that `ChannelsService` actually calls. */
...
/** The Postgres driver decorates the error with these fields at runtime. */
interface PostgresDriverError extends QueryFailedError {
  code: string;
  detail: string;
}
...
function makeDataSource(manager: MockEntityManager): DataSource {
  return {
    transaction: jest.fn(
      (cb: (manager: MockEntityManager) => Promise<unknown>) => cb(manager),
    ),
